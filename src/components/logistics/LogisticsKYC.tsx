  ...
  if (kyc && (kyc.status === "verified" || kyc.status === "pending")) {
    ...
    return (
      <div className="max-w-2xl mx-auto py-10">
        <Card className="rounded-xl border-none shadow-2xl shadow-black/[0.03] overflow-hidden">
          <div className={cn(
            ...
            <div className={cn(
              "p-4 rounded-xl",
              isVerified ? "bg-green-500 text-white" : "bg-amber-500 text-white"
            )}>
              {isVerified ? <CheckCircle2 size={32} /> : <Clock size={32} />}
            </div>
          </div>
          <CardHeader className="text-center p-8 pt-6">
            <CardTitle className="text-3xl font-black tracking-tight uppercase">
              {isVerified ? "Profile Verified" : "Review in Progress"}
            </CardTitle>
            <CardDescription className="text-base font-medium max-w-sm mx-auto">
              {isVerified 
                ? "Your account is fully verified. You can now claim any available missions in your zone." 
                : "Our safety team is currently reviewing your documents. This usually takes 24-48 hours."}
            </CardDescription>
          </CardHeader>
          <CardContent className="p-8 pt-0 flex flex-col items-center gap-6">
            <div className="grid grid-cols-2 gap-4 w-full max-w-md">
              <div className="p-4 rounded-xl bg-muted/30 border border-black/[0.03]">
                <p className="text-[10px] font-black uppercase text-muted-foreground mb-1 tracking-widest">Full Name</p>
                <p className="text-sm font-bold truncate">{kyc.full_name}</p>
              </div>
              <div className="p-4 rounded-xl bg-muted/30 border border-black/[0.03]">
                <p className="text-[10px] font-black uppercase text-muted-foreground mb-1 tracking-widest">Phone</p>
                <p className="text-sm font-bold truncate">{kyc.phone_number}</p>
              </div>
              <div className="p-4 rounded-xl bg-muted/30 border border-black/[0.03]">
                <p className="text-[10px] font-black uppercase text-muted-foreground mb-1 tracking-widest">NIN Number</p>
                <p className="text-sm font-bold">{kyc.nin_number || '€”'}</p>
              </div>
            </div>
